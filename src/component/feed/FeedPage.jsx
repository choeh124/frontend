...
export default function FeedPage() {
    ...
    useEffect(() => {
        const findFeed = async () => {
            const response = await axios.get(`http://127.0.0.1:8000/api/groups/${groupId}/feeds`);
    ...
    const handleSubmin = (e) => {
        ...
        try {
            const feedContentWrite = async () => {
                const response = await axios.post(`http://127.0.0.1:8000/api/groups/${groupId}/feeds`,
                    { content },
                    {
                        headers: {
                ...
                <div className="FeedContent">
                    <div className="FeedInfo">
                        ...
                        <form onSubmit={handleSubmin}>
                            <input onChange={(e) => { e.preventDefault(); setContent(e.target.value) }} type="text" />
                            ...
                            return (
                                <div key={feed.id} className='FeedListContent'>
                                    <div className='FeedDetail'>
                                        ...
                                        {editingContentId === feed.id ? (
                                            <div>
                                                ...
                                                />
                                                <button onClick={() => saveEditing(feed.id)}>저장</button>
                                    ...
                                    <div className='FeedButton'>
                                        {editingContentId !== feed.id && (
                                            <button onClick={() => startEditing(feed.id, feed.content)} >수정</button>
                                        )}
                                        {/* 피드삭제 */}
                                        <button
                                            onClick={async () => {
                                                if (window.confirm("피드를 삭제하기겠습니까?")) {
                                                    try {
                                                        ...
                                                        await axios.delete(`localhost:8000/api/groups/${groupId}/feeds/${feed.id}`, {
                                                            headers: {
                                                                Authorization: authorization
                                                            },
                                                        });
                                                        setFeedlist(feedlist.filter((feeds) => feeds.id !== feed.id));
                                                    } catch (e) { };
                                                }
                                            }}
